...
function Builder(dir, parent) {
  ...
  this._hooks = {};
  this._files = {};
  this._json = parent ? parent._json : {};
  this._js = '';
  this.urlPrefix = '';
  this.copy = false;
  this.dir = dir;
  ...
  this.basename = this.root ? this.config.name : basename(dir);
...
Builder.prototype.local = function(){
  var out = {};
  for (var i = 0; i < this.config.local.length; i += 1) {
    var name = this.config.local[i];
    out[name] = '*';
  }
  return out;
};
...
Builder.prototype.lookup = function(name, fn){
  ...
  function next() {
    ...
    fs.exists(join(dir, 'component.json'), function (exists) {
      cache[key] = exists ? dir : 'NOEXIST';
      cb();
    });
  }
...
Builder.prototype.ignoring = function(name, type){
  if (!type) {
    return this.ignoring(name, 'scripts') && this.ignoring(name, 'styles');
  }
...
Builder.prototype.json = function(){
  ...
  if (!this._json[path]) {
    ...
    this._json[path] = fs.readFileSync(path, 'utf8');
  } else {
    debug('cache hit %s', path);
  }

  var obj, str = this._json[path];
...
Builder.prototype.buildAliases = function(fn){
    ...
    Object.keys(self.dependencies()).forEach(function(dep){
      batch.push(function(done){
        ...
        self.lookup(dep, function(err, dir){
          ...
          var aliases = [];
          for (var i = 0; i < conf.scripts.length; i += 1) {
            var script = conf.scripts[i];
            var alias = self.root ? self.config.name + '/deps/' + name + '/' + script
              : self.basename + '/deps/' + name + '/' + script;

            aliases.push(builder.alias(alias, script));
          }

          if (main) {
            var alias = self.root ? self.config.name + '/deps/' + name + '/index.js'
          ...
          if (self._aliases[dir]) {
            debug('already aliased %s', dir);
            return done(null, aliases);
          }

          builder.buildAliases(function(err, str){
            if (err) return done(err);
            self._aliases[dir] = aliases;
            done(null, aliases + '\n' + str);
          });
        });
      });
  ...
  batch.end(function(err, res){
    if (err) return fn(err);
    var name = self.root ? self.config.name : self.basename;
